...
def db_start_location(county, state, city):
    ...
    cur = conn.cursor()
    if county:
        cur.execute("SELECT index, coord_lat, coord_long, adjusted_visit_length, ranking, review_score, num_reviews FROM poi_detail_table WHERE county = '%s' AND state = '%s';" % (county.upper(), state.title()))
    else:
        cur.execute("SELECT index, coord_lat, coord_long, adjusted_visit_length, ranking, review_score, num_reviews FROM poi_detail_table WHERE city = '%s' AND state = '%s';" % (city.title(), state.title()))
    a = cur.fetchall()
    conn.close()
    return np.array(a)
...
def db_day_trip_details(event_ids, i):
    ...
    for event_id in event_ids:
        ...
        a = cur.fetchone()
        details.append({'id': a[0],'name': a[1],'address': a[2], 'day': i, 'coord_lat': a[3], 'coord_long': a[4], 'city': a[5], 'state': a[6], 'icon_url': a[7], 'check_full_address': a[8], 'poi_type': a[9], 'adjusted_visit_length': a[10]})
    conn.close()
    return details
