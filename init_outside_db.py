...
def init__outside_db_tables():
    ...
    outside_route_table = pd.DataFrame(columns =['outside_route_id', 'full_day', 'regular', 'origin_city', \
                        'origin_state', 'target_direction', 'details', 'event_type', 'event_ids'])
    outside_trip_table = pd.DataFrame(columns = ['user_id', 'outside_trip_id', 'outside_route_ids', 'event_id_lst', 'origin_city', \
                        'origin_state', 'target_direction', 'n_days', 'regular', 'full_day', 'details'])
    google_city_to_poi_table = pd.DataFrame(columns = ['city_to_poi_id', 'city_id', 'origin_city', 'origin_state', \
                        ...
                        'dest_coord_long','orig_coords','dest_coords', 'google_driving_url', 'google_walking_url',\
                         'tdriving_result', 'walking_result', 'city_to_poi_driving_time', 'city_to_poi_walking_time'])
    ...
    google_city_to_poi_table.loc[0] = [999999, 999999, 'Test','Test','Test','Test', 999999, 12.345, -123.45, ]
